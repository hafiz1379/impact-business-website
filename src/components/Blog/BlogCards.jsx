...
import BlogCard from "./BlogCard"; // Assuming BlogCard is imported from the correct location
  ...
  return (
    <div className="px-4 py-12 md:px-12 lg:px-16 space-y-2 grid md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
      {blogItems.map((item, index) => (
        <BlogCard
          key={item.name + index} // Using a combination of name and index as the key
          img1={item.img1}
          job={item.job}
          desc={item.desc}
          img2={item.img2}
